...
module.exports = exports = function (socket, config) {
    ...
    function onConnection(socket) {
			...
			console.debug('join-broadcast - user: ' + JSON.stringify(user));
				...
				if (!users[user.userid]) {
					...
					users[user.userid] = {
						...
						maxRelayLimitPerUser: params.maxRelayLimitPerUser,
						relayReceivers: [],
						receivingFrom: null,
						canRelay: false,
				...
				var relayUser = getFirstAvailableBroadcaster(user.broadcastId, params.maxRelayLimitPerUser);
        ...
        socket.on('join-room', function (arg, callback) {
            ...
            try {
				// 临时注释掉下面一行，以便于随意两两互连. johnny xu,2021/3/29
                // closeOrShiftRoom();
